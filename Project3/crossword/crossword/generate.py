...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for word_x in set(self.domains[x]):
            # Check if there is no word in y's domain that satisfies the overlap
    ...
    def ac3(self, arcs=None):
        ...
        """
        # Initialize queue with all arcs if none are provided
        ...
        while queue:
            x, y = queue.popleft()
            if self.revise(x, y):
                if not self.domains[x]:  # If domain is empty, return False
                    return False
    ...
    def backtrack(self, assignment):
        ...
        for value in self.order_domain_values(var, assignment):
            ...
            if self.consistent(new_assignment):
                # Enforce arc consistency with the new assignment
                arcs = [(neighbor, var) for neighbor in self.crossword.neighbors(var)]
                if self.ac3(arcs):  # Only continue if arc consistency is maintained
                    result = self.backtrack(new_assignment)
                    if result:
                        return result
            # If not consistent or no solution found, continue to the next value
        return None
